...
void ServerManager::onNewConnection()
{
    ...
    QTcpSocket* clientConnection = m_server.nextPendingConnection(); //получили указатель на сокет
    if (clientConnection)
    {
...
void ServerManager::onDataArrived()
{
    ...
    forever {
        ...
        {
            break; //когда дошли до состояня, когда в буфере нет ничего валидного
        }
